	...
	<link href="css/index.css" type="text/css" rel="stylesheet"> 
  	...
  	<script type="text/javascript">
		$(function () {
			...
			$("#last").click(function () {
				var pages = parseInt($("#pages").html());
				location.href="/product?page="+pages;
			})
		})
	</script>
  </head>
  ...
  <%@ include file="top.jsp" %>
  	...
  	<div id="main">
		<div class="navigation">
				...
				<div id="readerBlock">欢迎回来&nbsp;:<a href="/product?method=findAllBorrow&page=1">${user.name }</a>&nbsp;<a href="/logout">注销</a></div>
		</div>
		<div class="img_block">
		...
		<table class="table" cellspacing="0">
			...
			<c:forEach items="${list}" var="product">
				<tr>
					<td>${product.id}</td>
					<td>${product.name}</td>
					<td>${product.lessor}</td>
					<td>${product.manufacturer}</td>
					<td>${product.rentprice}</td>
					<td>${product.price}</td>
					<td>${product.productCase.name}</td>
					<td>
						<a href="/product?method=addBorrow&productid=${product.id}">租用</a>
					</td>
				</tr>
			</c:forEach>
